...
function todos($array)
{
  ...
  foreach ($functions as $index => $function) {
    ...
    $output .= "<tr><td>$function_names[$index]</td><td>$resultado[0]</td><td>$resultado[1]</td></tr>";
  }
  return $output . "</table>";
}
...
function render_get()
{
  ...
  if ($metodo_ordenacion === 'todos') {
    $output .= '<div class="flex"><div class="flex center column"><span>Array generado: ';

    foreach ($array as $key => $value) {
      $output .= "$value ";
    }

    $output .= '</span><p><strong>Comparación de los tiempos de ejecución y número de iteraciones de los diferentes algoritmos de ordenación:</strong></p>';
...
function uploadTxt()
{
  ...
  if ($upload_ok == 0) {
    echo '<p class="error">Carga de fichero fallida.</p>';
  } else {
    if (move_uploaded_file($_FILES['fichero']['tmp_name'], $target_file)) {
      ...
      return '<p class="error">Lo sentimos, ha habido un error en la carga del archivo.</p>';
    }
  }
}
