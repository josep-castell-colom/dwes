...
<input type="text" name="extracto" id="extracto" placeholder="{{ __('Post\'s extract') }}" class="text-gray-900"
    value="{{ old('extracto', $post->extracto ?? '') }}">
@error('extracto')
...
<textarea name="contenido" id="contenido" placeholder="{{ __('Blah, blah, blah...') }}" class="text-gray-900">{{ old('contenido', $post->contenido ?? '') }}</textarea>
@error('contenido')
